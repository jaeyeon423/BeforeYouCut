...
export default async function CategoryPage({ searchParams }) {
  ...
  const { items, hasMore, total } = await getCategoryProducts(cat, 0, 20, filter);

  return (
    <>
      <TabHeader title="카테고리" bordered />
      <CategoryScreen cat={cat} initialItems={items} initialHasMore={hasMore} total={total} filter={filter} />
    </>
  );
}
